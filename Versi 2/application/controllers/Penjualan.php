...
class Penjualan extends CI_Controller {
	...
	public function proses_pembayaran()
	{
		...
		foreach ($this->cart->contents() as $item ) {
			...
			if ($jumlah_bayar > $item['subtotal']){
				$detail_penjualan = $this->detail_penjualan_model->store($dataDetailPenjualan);
				$trigger_barang = $this->detail_penjualan_model->update_stok($item['id'], $item['qty']);
				...
				$this->session->set_flashdata('pesan', 'Total Bayar yang anda masukkan kurang dari Total Harga');
				redirect('penjualan/pembayaran');
			}
